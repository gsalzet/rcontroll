...
```{r fullsimG3}
autoplot(sim, what = "distribution", variables = c("dbh", "height", "LA", "LMA"))
```
...
```{r fullstackG2, fig.height=4}
autoplot(sim_stack, what = "spatial")
```
...
```{r}
autoplot(TROLLv3_output, what = "temporal")
```
...
```{r}
autoplot(TROLLv3_output, what = "spatial", variables = "age")
```
...
list("TROLLv3_output" = TROLLv3_output@ecosystem,
  ...
  theme_bw() +
  xlab("Time (years)") +
  ylab(expression(Abovergound~biomass~(AGB~Kg~ha^{-1})))
```
